...
SCRIPT_TAG="${SCRIPT_TAG:-pairwise_worker}"
GPU_ID="${GPU_ID:-1}"
...
PAIRWISE_LOSS="${PAIRWISE_LOSS:-log_sigmoid}"
...
if [[ -n "${PAIRWISE_WEIGHTS:-}" ]]; then
    read -r -a PAIRWISE_WEIGHT_LIST <<< "${PAIRWISE_WEIGHTS}"
else
    PAIRWISE_WEIGHT_LIST=(0.1 0.2 0.4 0.6)
fi
...
OUTPUT_PREFIX="${NEWS_ENCODER}_${USER_ENCODER}_PAIRWISE_${PAIRWISE_LOSS}"
STATE_ROOT="sweep_state/${NEWS_ENCODER,,}_${USER_ENCODER,,}_pairwise_${PAIRWISE_LOSS}"
LOCK_ROOT="${STATE_ROOT}/locks"
DONE_ROOT="${STATE_ROOT}/done"
FAILED_ROOT="${STATE_ROOT}/failed"
WORKER_LOG="logs/${SCRIPT_TAG}_pairwise_worker.log"
CURRENT_LOCK_DIR=""
CURRENT_WEIGHT=""
ACTIVE_CHILD_PID=""
...
refresh_summary() {
    set +e
    "${PYTHON_BIN}" summarize_pairwise_sweep.py \
        --news-encoder "${NEWS_ENCODER}" \
        --user-encoder "${USER_ENCODER}" \
        --dataset "${DATASET}" \
        --mind-size "${MIND_SIZE}" \
        --pairwise-loss "${PAIRWISE_LOSS}" \
        --num-runs "${NUM_RUNS}" \
        --weights "${PAIRWISE_WEIGHT_LIST[@]}" \
        >> "${WORKER_LOG}" 2>&1
    set -e
}
...
exp_tag_for_weight() {
    printf 'pairwise_%s_w_%s' "${PAIRWISE_LOSS}" "$(safe_weight "$1")"
}
...
claim_next_weight() {
    local weight
    local safe_pairwise_w
    local lock_dir
    local owner_tag

    for weight in "${PAIRWISE_WEIGHT_LIST[@]}"; do
        if weight_done "${weight}"; then
            log_msg "skip completed weight=${weight}"
            continue
        fi

        if (( RETRY_FAILED == 0 )) && weight_failed "${weight}"; then
            log_msg "skip failed weight=${weight} retry_failed=${RETRY_FAILED}"
            continue
        fi
        ...
        safe_pairwise_w="$(safe_weight "${weight}")"
        lock_dir="${LOCK_ROOT}/${safe_pairwise_w}.lock"
        ...
        while true; do
            if mkdir "${lock_dir}" 2>/dev/null; then
                ...
                if weight_done "${weight}"; then
                    log_msg "weight=${weight} finished while claiming; releasing lock"
                    release_current_lock
                    break
                fi

                log_msg "claimed weight=${weight} on gpu=${GPU_ID}"
                return 0
            fi

            if ! lock_owner_alive "${lock_dir}"; then
                log_msg "remove stale lock weight=${weight}"
                rm -rf "${lock_dir}"
                continue
            fi
            ...
            log_msg "skip locked weight=${weight} owner=${owner_tag}"
            break
        done
    done
...
run_claimed_weight() {
    ...
    if [[ "${PAIRWISE_LOSS}" == "margin" ]]; then
        extra_args+=(--prev_nonclick_pairwise_margin "${PAIRWISE_MARGIN}")
    fi

    log_msg "launch weight=${weight} gpu=${GPU_ID} exp_tag=${exp_tag} output=${output_name} loss=${PAIRWISE_LOSS}"
    ...
    "${PYTHON_BIN}" main.py \
        ...
        --prev_nonclick_pairwise_weight "${weight}" \
        --prev_nonclick_pairwise_loss "${PAIRWISE_LOSS}" \
        "${extra_args[@]}" \
        --exp_tag "${exp_tag}" \
    ...
    if (( status == 0 )) && (( dev_count >= NUM_RUNS )) && (( test_count >= NUM_RUNS )); then
        ...
        log_msg "done weight=${weight} dev_runs=${dev_count} test_runs=${test_count}"
        return 0
    fi

    mark_weight_failed "${weight}" "${log_file}" "${status}"
    log_msg "failed weight=${weight} status=${status} dev_runs=${dev_count} test_runs=${test_count} log=${log_file}"
    return 1
}
...
main() {
    ...
    log_msg "worker start gpu=${GPU_ID} batch_size=${BATCH_SIZE} num_runs=${NUM_RUNS} pairwise_loss=${PAIRWISE_LOSS}"
    preflight
    refresh_summary
    ...
    log_msg "worker finished; no remaining weight"
}
